...
@secure_files_bp.route("/secure-files/list", methods=["GET", "POST"], strict_slashes=False)
@login_required
def list_files():
...
@secure_files_bp.route("/secure-files/userInfo", methods=["GET", "POST"], strict_slashes=False)
@login_required
def list_user_info():
    user_name = get_user_name(req=request)
