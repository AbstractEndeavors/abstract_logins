...
def admin_login_prompt():
    ...
    window = get_AdminLoginWindow()
    window.show()
    app.exec_()
    return window.result
def get_UserManagementWindow(admin_user):
    from PyQt5.QtWidgets import QMainWindow
        ...
        def handle_ok(self):
            ...
            if existing_row is None:
                ...
                if not pwd_input:
                    ...
                    self.close()
    return UserManagementWindow(admin_user)
def edit_users():
    ...
    app = QApplication(sys.argv)
    window = get_UserManagementWindow(admin_user)
    window.show()
    app.exec_()
